        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              <div className={`text-4xl font-bold ${config.textColor}`}>{(confidence ?? 0).toFixed(0)}%</div>
          ...
          <div className={`${config.bgColor} rounded-lg p-4 mb-4`}>
            <p className={`${config.textColor} font-medium`}>{reasoning?.summary ?? "No summary available"}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-lg p-4 border">
              ...
              <div className="text-xl font-bold text-gray-900">${(getTargetPrice() ?? 0).toFixed(2)}</div>
              <div className="text-sm text-gray-500">
                {(((getTargetPrice() ?? 0) - (currentPrice ?? 0)) / ((currentPrice ?? 1)) * 100).toFixed(1)}% upside
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg p-4 border">
              ...
              <div className="text-xl font-bold text-gray-900">${(getStopLoss() ?? 0).toFixed(2)}</div>
              <div className="text-sm text-gray-500">
                {(((getStopLoss() ?? 0) - (currentPrice ?? 0)) / ((currentPrice ?? 1)) * 100).toFixed(1)}% downside
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Set stop-loss at ${(getStopLoss() ?? 0).toFixed(2)} and monitor position size relative to portfolio
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
